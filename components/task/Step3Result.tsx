...
export function Step3Result({
  ...
  const handleDownload = async (options: DownloadOptions) => {
    ...
    try {
      ...
      a.href = url
      a.download = `Tugas-${formData.course_name.replace(/\s+/g, '-')}.${extension}`
      a.click()
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Button onClick={() => setShowDownloadModal(true)} disabled={isDownloading} className="gap-2 bg-emerald-600 hover:bg-emerald-700">
          {isDownloading ? (
